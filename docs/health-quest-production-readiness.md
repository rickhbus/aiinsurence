...
Apply Supabase migrations in numeric order. Health Quest retention requires `013` through `020` after the Health OS foundation:
...
- Full production readiness still requires staging validation, real RLS verification, monitoring checks, and load testing.
